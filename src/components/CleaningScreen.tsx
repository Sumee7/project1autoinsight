...
export default function CleaningScreen({
  ...
  useEffect(() => {
    setCleanedRows(dataSummary.rows);
  }, [dataSummary.rows]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex">
      <div className="flex-1 p-6 overflow-y-auto">
        <div className="max-w-5xl mx-auto">
          <div className="mb-6">
            <h1 className="text-3xl font-bold text-white mb-2">Data Cleaning Report</h1>
            <p className="text-gray-300">Review and clean your data before analysis</p>
          </div>

          {cleaned && (
            <div className="mb-6 bg-green-500/20 border border-green-500/50 rounded-lg p-4 flex items-center gap-3">
              <CheckCircle className="w-5 h-5 text-green-400 flex-shrink-0" />
              <p className="text-green-200 font-medium">Data cleaned successfully.</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            <div className="bg-gray-800/50 backdrop-blur border border-gray-700/50 rounded-xl p-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="font-semibold text-white">Rows</h3>
              </div>
              <p className="text-3xl font-bold text-white">{cleanedRows.toLocaleString()}</p>
            </div>
            ...
            <div className="bg-gray-800/50 backdrop-blur border border-gray-700/50 rounded-xl p-6">
              ...
              <p className="text-3xl font-bold text-white">{dataSummary.columns}</p>
            </div>

            <div className="bg-gray-800/50 backdrop-blur border border-gray-700/50 rounded-xl p-6">
              <div className="flex items-center gap-3 mb-2">
                <Database className="w-5 h-5 text-blue-400" />
                <h3 className="font-semibold text-white">Column Types</h3>
              </div>
              <div className="flex gap-2 mt-2 flex-wrap">
                {Array.from(new Set(dataSummary.columnDetails.map((c) => c.type))).map((type) => (
                  <span
                    key={type}
                    className="px-2.5 py-1 bg-blue-500/20 text-blue-300 text-sm rounded-full font-medium border border-blue-500/30"
                  >
                    {type}
                  </span>
                ))}
          ...
          <div className="bg-gray-800/50 backdrop-blur border border-gray-700/50 rounded-xl p-6 mb-6">
            <div className="flex items-center gap-3 mb-6">
              <AlertCircle className="w-6 h-6 text-orange-400" />
              <h2 className="text-xl font-semibold text-white">Cleaning Issues</h2>
            </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                {cleanedIssues.missingValues.length === 0 ? (
                  <div className="p-4 bg-green-500/20 rounded-lg border border-green-500/30 flex items-center gap-2">
                    <CheckCircle className="w-5 h-5 text-green-400" />
                    <p className="text-green-300 font-medium">No missing values detected</p>
                  </div>
                ) : (
                    ...
                    {cleanedIssues.missingValues.map((col) => (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-sm text-gray-400">Type: {col.type}</p>
                        </div>
                        <span className="px-3 py-1 bg-red-500/20 text-red-300 text-sm font-semibold rounded-full border border-red-500/30">
                          {col.missing} missing
                        </span>
                      </div>
                    ))}
              ...
              <div>
                ...
                {cleanedIssues.invalidTypes.length === 0 ? (
                  <div className="p-4 bg-green-500/20 rounded-lg border border-green-500/30 flex items-center gap-2">
                    <CheckCircle className="w-5 h-5 text-green-400" />
                    <p className="text-green-300 font-medium">No invalid type values detected</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {cleanedIssues.invalidTypes.map((col) => (
                      ...
                      >
                        <div>
                          <p className="font-medium text-white">{col.name}</p>
                          <p className="text-sm text-gray-400">Expected: {col.type}</p>
                        </div>
              ...
              <div>
                ...
                {cleanedIssues.duplicates === 0 ? (
                  <div className="p-4 bg-green-500/20 rounded-lg border border-green-500/30 flex items-center gap-2">
                    <CheckCircle className="w-5 h-5 text-green-400" />
                    <p className="text-green-300 font-medium">No duplicate rows detected</p>
                  </div>
                ) : (
                  <div className="p-3 bg-gray-700/50 rounded-lg border border-gray-600/50">
                    <p className="text-white">
                      <span className="font-semibold">{cleanedIssues.duplicates}</span> duplicate rows found
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="bg-gray-800/50 backdrop-blur border border-gray-700/50 rounded-xl p-6 mb-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h2 className="text-xl font-semibold text-white">Cleaning Actions</h2>
                <p className="text-sm text-gray-400 mt-1">
                  Clean issues by imputing missing values, correcting invalid types, and removing duplicates
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className="flex items-center gap-2 bg-blue-600 text-white font-semibold px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors shadow-lg"
              >
              ...
              <button
                ...
                className="bg-gray-700 text-gray-100 font-medium px-6 py-3 rounded-lg border border-gray-600 hover:bg-gray-600 transition-colors"
              >
                Fix Missing Only
              </button>
              ...
              <button
                ...
                className="flex items-center gap-2 bg-green-600 text-white font-medium px-6 py-3 rounded-lg hover:bg-green-700 transition-colors shadow-lg ml-auto"
              >
            ...
            <div className="mt-4 p-3 bg-blue-500/20 rounded-lg border border-blue-500/30">
              <p className="text-sm text-gray-300">
                <span className="font-semibold text-blue-300">Cleaning methodology:</span> Missing values are imputed using mean
                (numeric) or mode (categorical). Invalid types are corrected to match expected format. Only duplicate rows are removed.
              </p>
            </div>
          </div>
